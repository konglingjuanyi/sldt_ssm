...
public class LdapAuthenticationHandler extends AbstractUsernamePasswordAuthenticationHandler {
    ...
    protected HandlerResult authenticateUsernamePasswordInternal(final UsernamePasswordCredential upc)
        ...
        String pdsAuthErrCode=getPdsLoginErrorCode(upc.getUsername());
        FailedLoginException fle=getPdsErrorException(pdsAuthErrCode);
        if(fle!=null){
        	throw fle;
        }else{
    ...
    //zzg 20141026
    //创建异常类
   private FailedLoginException getPdsErrorException(String authErrCode){
	   String errorClassName="org.jasig.cas.authentication.pdserr.cgb.teller.PdsException"+authErrCode+"";
	   try {
		  Class clazz=Class.forName(errorClassName);
		  FailedLoginException retErr=(FailedLoginException) clazz.newInstance();
		  return retErr;
	   } catch (ClassNotFoundException e) {
			e.printStackTrace();
			logger.info("未定义pds错误代码["+authErrCode+"]对应的类:"+errorClassName);
   ...
   //zzg 20141026
   //计算pds的错误代码，来源于柜面等集中验证服务器的反馈代码
   private String getPdsLoginErrorCode(String userId){
    	
    	//modify by chenbo 2012-09-18
    	String authBadCode ="";// "error.authentication.credentials.bad";
    	//获取登录账号
      //  String tempStr = credentials.toString();
      //  String userId = tempStr.substring(tempStr.indexOf("username:")+9, tempStr.indexOf("]"));
        userId = userId.trim();
        logger.debug("--------------------userId-------1-------------"+userId);
        System.out.println("------------------userId：-------------------"+userId);
           
     // ApplicationContext beanTool =  new ClassPathXmlApplicationContext("authenpasw.xml");
        IAuthenticateLogInfoService authloginfoService= (IAuthenticateLogInfoService)BeanUtils.getBean("authenticateLogInfoService");
		AuthLogInfoBean authLogInfo= authloginfoService.getAuthLogInfoByUserId(userId);
		if(authLogInfo!=null){
        	   if(!"".equals(authLogInfo.getErrorCode().trim()) && !"0000".equals(authLogInfo.getErrorCode().trim())){//获取CODE
        		  //authBadCode="error.authentication.credentials."+authLogInfo.getErrorCode().trim();
        		   authBadCode=authLogInfo.getErrorCode().trim();
        	   }
            }
		return authBadCode;
    }
