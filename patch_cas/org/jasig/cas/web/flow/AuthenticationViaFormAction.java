...
public class AuthenticationViaFormAction {
    ...
    public final Event submit(final RequestContext context, final Credential credential,
        ...
        try {
            ...
            ///////////////////////////////////////////////
            //zzg 20141026 该处需要检查是否需要强行修改密码
            //考虑到CGB SSO应该无需考虑强制修改密码的问题，这里可以设置标志位，禁用下密码修改检查逻辑
            //保留检查用于支持柜面终端的密码修改逻辑
            boolean isNeedCheckModifyPassword=true;
            
//            final HttpServletRequest request = WebUtils.getHttpServletRequest(context);
//            String client_name=request.getParameter("client_name");
//            if(client_name!=null&&client_name.length()>0){//有明确的客户端验证参数
//            	 isNeedCheckModifyPassword=false;//不对client_name参数的请求进行密码修改验证
//            }
          
            if(isNeedCheckModifyPassword){
	            //添加密码修改引导逻辑，如果需要修改密码，则引导进入密码修改页面，否则进入系统（包括即将过期情况）
	            //获取登录账号
	         ...
	         // ApplicationContext beanTool =  new ClassPathXmlApplicationContext("authenpasw.xml");
	            ...
	            //modify by chenbo 因未集成pds ldap，无pds审计日志，是否修改密码的机制先注释
	            /*AuthLogInfoBean authLogInfo= authloginfoService.getAuthLogInfoByUserId(userId);
	            if(authLogInfo!=null){
	        	  if("1".equals(authLogInfo.getPwdmodFlag().trim())){//表示需修改密码
	        		   //if("无".equals(authLogInfo.getPwdmodFlag().trim())){//表示需修改密码	  --temp for test
	        		   return newEvent("modifypas");
	        	   }
	            } */  
            }
            /////////////////////////////////////////////
