 ...
 * @since 1.4.0
 */
public class HzbankCasClient extends BaseClient<CasCredentials, CasProfile> {
    
    protected static final Logger logger = LoggerFactory.getLogger(HzbankCasClient.class);
    ...
    protected BaseClient<CasCredentials, CasProfile> newClient() {
        final HzbankCasClient casClient = new HzbankCasClient();
    ...
    protected CasCredentials retrieveCredentials(final WebContext context) throws 
        ...
        if (context.getRequestParameter(SERVICE_TICKET_PARAMETER)!=null) {
    ...
    protected CasProfile retrieveUserProfile(final CasCredentials credentials) {
        ...
        try {
        	System.out.println("[bug20161012_uas]ticket["+ticket+"]callbackUrl["+this.callbackUrl+"]");
            final Assertion assertion = this.ticketValidator.validate(ticket, this.callbackUrl);
            final AttributePrincipal principal = assertion.getPrincipal();
            logger.debug("principal : {}", principal);
            System.out.println("[bug20161012_uas]ticket["+ticket+"]principal["+principal+"]");
        	...
        	e.printStackTrace();
        	System.out.println("[bug20161012_uas]ticket["+ticket+"],error:"+e.getMessage());
