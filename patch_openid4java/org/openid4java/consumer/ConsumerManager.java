...
public class ConsumerManager
{
    ...
    public boolean verifyReturnTo(String receivingUrl, AuthSuccess response)
    {
        ...
        //zzg 20141015 暂时不验证参数  
        // 因 openId 2.0以下 版本 在 验证参数时会 在 returnTo后追加两个参数（&openid.rpnonce和&openid.rpsig）
        // 但是实际的 callbackUrl中不含有 这两个参数，以至验证失败，无法跳转至成功页面。该版本不做此类判断所以可以成功跳转至成功页面（即获取到验证ticket的cas server成功页面）。
        // OpenId 2.0版本不存在此问题。

        boolean isNeedVerifyParams=false;
        if(isNeedVerifyParams){
