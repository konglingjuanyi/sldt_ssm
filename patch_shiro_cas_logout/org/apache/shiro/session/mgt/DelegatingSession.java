...
public class DelegatingSession implements Session, Serializable {
    ...
    public Object getAttribute(Object attributeKey) throws InvalidSessionException {
    	Object retObj=null;
    	//zzg 20160707 cas单点登出逻辑，会引起该出异常，这里兼容该错误
    	//org.apache.shiro.session.UnknownSessionException: There is no session with id [1d69d3f2-99b2-415f-9886-12ed61f18939]
    			
    	try{
    	   retObj=sessionManager.getAttribute(this.key, attributeKey);
    	}catch(org.apache.shiro.session.UnknownSessionException use){
    		String msg=use.getMessage();
    		//org.apache.shiro.session.UnknownSessionException: There is no session with id [876cdd4d-0b1a-43bc-87c7-007e019bfd3d]
    		if(msg!=null&&msg.indexOf("There is no session with id")!=-1){
    			//忽略
    			//use.printStackTrace();
