			...
			handler : function() {
				...
				if(ids.length>0){
					for(var i=0;i<ids.length;i++){
						...
						if(rowData.quesSolutionState=="已发布"){
							quesIds+=(rowData.quesId+",");							
						}
					}
					if(quesIds.length>0){
						checkState(quesIds,"yes");						
					}else{						
						layer.msg('只能选择已发布的解决方案！', {shift: 6});						
					}
					//打开修改窗口
				}else{
...
//	var width=$('#managerContent1').width()-10;
	...
	jQuery(grid_selector).jqGrid({
		...
		postData: {"quesSolutionState":"已发布",
    		"quesType":"重大问题"},
		datatype: "json",
		mtype: "POST",
		height: "230",
...
function checkState(quesIds,flag){
	$.ajax({ 
		type: 'post', 
        url: context+"/dealAction.do?method=checkState", 
